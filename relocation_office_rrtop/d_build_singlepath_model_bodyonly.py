...
def build_1path_vgg_bodyonly_model(img_height=448,
                                   weights='imagenet',
    ...
    for i in np.arange(nb_fc_hidden_layer):
        body_path_x = Dense(
            nb_fc_hidden_node,
            name='fc_dense{}_body'.format(i+1),
            activation='linear',
        ...
        if is_bn_enabled:
            body_path_x = BatchNormalization(name='fc_bn{}_body'.format(i+1))(body_path_x)
        body_path_x = Activation('relu', name='fc_act{}_body'.format(i+1))(body_path_x)
        if is_do_enabled:
            body_path_x = Dropout(dropout_ratio, name='fc_do{}_body'.format(i+1))(body_path_x)

    x = Dense(2,
              name='fc_dense{}_body'.format(nb_fc_hidden_layer + 1),
              activation='linear',
              W_learning_rate_multiplier=learning_rate_multiplier,
              b_learning_rate_multiplier=learning_rate_multiplier*2,
              W_regularizer=l1l2(l1=l1_regularization, l2=l2_regularization)
    ...
    model = Model(inputs, x, name='vgg_bodyonly_1path_model')
...
if __name__ == '__main__':
    ...
    plot(model_stacked,
         './models/model_1path_body_only_20170331.png',
