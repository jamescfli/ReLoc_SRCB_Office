...
if __name__ == '__main__':
    ...
    np.save(open('predicted_data/train_position_result.npy', 'w'), train_pos)
    np.save(open('predicted_data/test_position_result.npy', 'w'), test_pos)

    # load the xy pos
    train_pos = np.load(open('predicted_data/train_position_result.npy', 'r'))
    test_pos = np.load(open('predicted_data/test_position_result.npy', 'r'))
    ...
    test_pos_gt = np.loadtxt('datasets/train_test_split_480x1920_20161125/test_label_x{}.csv'
                             .format(label_scalar), dtype='float32', delimiter=',')
    ...
    train_loss_mse = (train_error**2).mean()    # 0.56459689 vs 5.1683 in keras 50epoch
    test_loss_mse = (test_error**2).mean()      # 0.62018335 vs 0.3101 in keras 50epoch
