...
class DConvolution2D(object):
    ...
    def __init__(self, layer):
        ...
        nb_up_filter = W.shape[0]   # W.shape[1] is the channel from last layer, e.g. input 3 = RGB for 'block1_conv1'
        nb_up_row = W.shape[2]
        nb_up_col = W.shape[3]
        input = Input(shape = layer.input_shape[1:])    # input_shape e.g. (None, 3, 224, 224)
        output = Convolution2D(
                nb_filter = nb_up_filter,
                nb_row = nb_up_row,
                nb_col = nb_up_col,
                border_mode = 'same',   # default 'valid'
                weights = [W, b]
                )(input)
        self.up_func = K.function([input, K.learning_phase()], output)

        # Flip W horizontally and vertically, and set down_func for DConvolution2D
        W = np.transpose(W, (1, 0, 2, 3))   # W shape used to be (nb_filter, nb_input_channel, nb_row, nb_col)
        W = W[:, :, ::-1, ::-1]
        nb_down_filter = W.shape[0]     # previous layer nb_input_channel
        nb_down_row = W.shape[2]
        nb_down_col = W.shape[3]
        b = np.zeros(nb_down_filter)    # deconv has no bias, b = 0
        ...
        self.down_func = K.function([input, K.learning_phase()], output)    # reverse function for deconv
    ...
    def up(self, data, learning_phase = 0):
        ...
        # Arguments
            ...
            learning_phase: learning_phase of Keras, 1 = train, 0 = test
        # Returns
            Convolved result
        '''
...
class DDense(object):
    ...
    def __init__(self, layer):
        ...
        flipped_weights = [W, b]    # W = W^T, b = 0
        input = Input(shape = self.output_shape[1:])
        output = Dense(output_dim = self.input_shape[1], weights = flipped_weights)(input)
...
class DPooling(object):
    '''
    A class to define forward and backward operation on Pooling, max-pooling only
    '''
    def __init__(self, layer):
        '''
        # Arguments
    ...
    def __max_unpooling_with_switch(self, input, switch):
        ...
        # Returns
            Unpooled result
        '''
        tile = np.ones((switch.shape[2] / input.shape[2], switch.shape[3] / input.shape[3]))
