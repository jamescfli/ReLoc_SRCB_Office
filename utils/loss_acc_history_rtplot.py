...
class LossAccRTPlot(Callback):
    def on_train_begin(self, logs={}):
        ...
        self.subplot_loss.set_xlim([1, 10])
    ...
    def _update_loss_plot(self):
        ...
        self.subplot_loss.set_ylim([np.floor(y_min*10.0)/10, np.floor(y_max*10.0)/10])
    ...
    def _update_acc_plot(self):
        ...
        y_min = min(self.train_accs)
        y_max = max(self.train_accs)
        self.subplot_loss.set_ylim([np.floor(y_min*10.0)/10, np.floor(y_max*10.0)/10])
        self.figure.canvas.draw()
...
if __name__ == '__main__':
    ...
    model = Sequential()
    model.add(Dense(1, input_dim=284, activation='sigmoid'))
    model.compile(optimizer='rmsprop',
                  loss='binary_crossentropy',
                  metrics=['accuracy'])

    data = np.random.random((100, 284))
